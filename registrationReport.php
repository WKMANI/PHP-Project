<!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
          content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Registration Report</title>
    <link rel="icon" href="images/emobilis.jpg">
    <link rel="stylesheet" href="css/styles.css">
    <link rel="stylesheet" href="bootstrap/css/bootstrap.css">
    <script src="bootstrap/js/jquery-3.4.1.js"></script>
    <script src="bootstrap/js/popper.min.js"></script>
    <script src="bootstrap/js/bootstrap.js"></script>
</head>
<body>
<br>
<div class="logo">
    <img src="images/emobilis.jpg" alt="Logo" width="224" height="103">
</div>
<br>
<div class="heading">
    <h3 class="text-center">Registration Report Details</h3>
</div>
<br>
<div class="container">
    <?php

    $conn = mysqli_connect("localhost", "root", "", "emobilis");

    if (!$conn){
        die("Connection Error: ". mysqli_error());
    }

    $course = mysqli_real_escape_string($conn,$_POST['course']);
    $dateFrom = mysqli_real_escape_string($conn,$_POST['dateFrom']);
    $dateTo = mysqli_real_escape_string($conn,$_POST['dateTo']);
    $result = mysqli_query($conn, "SELECT * FROM registration where course_enrolled = '$course' and reg_date>= '$dateFrom' and reg_date<= '$dateTo'");

    echo "<table class='table table-bordered table-striped table-responsive-sm'>
    <tr>
    <th>Student Admission Number</th>
    <th>First Name</th>
    <th>Other Name</th>
    <th>Gender</th>
    <th>Date of Birth</th>
    
    <th>Telephone Number</th>
    <th>Email</th>
    
    <th>Course Enrolled</th>
    
    <th>KCSE Grade</th>
    <th>Education Level</th>
    
    </tr>";

    while($row = mysqli_fetch_array($result))
    {
        echo "<tr>";
        echo "<td>". $row['studentAdm_No']."</td>";
        echo "<td>". $row['first_name']."</td>";
        echo "<td>". $row['other_name']."</td>";
        echo "<td>". $row['gender']."</td>";
        echo "<td>". $row['dob']."</td>";

        echo "<td>". $row['telephone_No']."</td>";
        echo "<td>". $row['email']."</td>";

        echo "<td>". $row['course_enrolled']."</td>";

        echo "<td>". $row['KCSE_Grade']."</td>";
        echo "<td>". $row['education_level']."</td>";

        echo "</tr>";
    }
    echo "</table>";
    mysqli_close($conn);

    ?>
</div>
</body>
</html>



